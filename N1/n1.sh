...
# Openwrt 
OP_ROOT_TGZ="openwrt/openwrt-armvirt-64-default-rootfs.tar.gz"
...
INST_SCRIPT="${PWD}/files/install.sh"
UPDATE_SCRIPT="${PWD}/files/update.sh"
...
if [ -f $BANNER ];then
    ...
    TODAY=$(date +%Y-%m-%d)
    echo " Packaged by gd772 on $TODAY" >> etc/banner
    echo >> etc/banner
fi
